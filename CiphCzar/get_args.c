...
status_t get_algo_args(algorithm_t* alg) {
    if (alg->additional_args == NULL) return status_ok();
    ...
    while (cur != NULL) {
        ...
        case INTEGER_ARG:
            ...
            if (!get_int_status.success) {
                fprintf(stderr, "Error: %s.", get_int_status.message);  // Currently this essentailly ruins the entire algorithm if it happens :p
